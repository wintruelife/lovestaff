package love.wintrue.com.lovestaff.widget.gesture;
...
public class GestureContentView extends ViewGroup {
	 ...
	 */
	public GestureContentView(Context context, boolean isVerify, String passWord,boolean showLine, GestureDrawline.GestureCallBack callBack) {
		super(context);
		screenDispaly = AppUtil.getScreenDispaly(context);
		blockWidth =(screenDispaly[0]- DensityUtil.dip2px(context,80))/3;
		...
		addChild();
		// 初始化一个可以画线的view
		gestureDrawline = new GestureDrawline(context, list, isVerify, passWord,showLine, callBack);
	}
	...
	private void addChild(){
		for (int i = 0; i < 9; i++) {
			...
			GesturePoint p = new GesturePoint(leftX, rightX, topY, bottomY, image,i+1,isShowLine);
			this.list.add(p);
		}
	}
	...
	public void setParentView(ViewGroup parent){
		// 得到屏幕的宽度
		int width = screenDispaly[0] - DensityUtil.dip2px(context,80);
