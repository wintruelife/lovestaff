	...
	public void startAutoFlowTimer(){
		handler = new Handler(){
			...
			public void handleMessage(Message msg) {
				if (handler != null && getChildCount() > 0) {
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if (velocityX > SNAP_VELOCITY && mCurrentScreen > 0) {
					// Fling hard enough to move mActionBarBackBtn
				...
				performItemClick(mLoadedViews.get(mCurrentBufferIndex), mCurrentAdapterIndex % mValideCount, 0);
			}
...
//			return false;
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			 ...
			 */
			int hPerceived = h + (mCurrentAdapterIndex%mValideCount - mCurrentBufferIndex)
					* getChildWidth();
	...
	public void setAdapter(Adapter adapter, int initialPosition) {
		...
		if (mValideCount == 0) {
			mValideCount = mAdapter.getCount();
		}
	...
	public void setSelection(int position) {
		...
		if (mIndicator != null) {
			mIndicator.onSwitched(currentView, mCurrentAdapterIndex%mValideCount);
		}
	...
	private void postViewSwitched(int direction) {
		...
		if (mIndicator != null && mLoadedViews != null && mCurrentBufferIndex < mLoadedViews.size()) {
			mIndicator.onSwitched(mLoadedViews.get(mCurrentBufferIndex), mCurrentAdapterIndex%mValideCount);
		}
		if (mViewSwitchListener != null && mLoadedViews != null && mCurrentBufferIndex < mLoadedViews.size()) {
			mViewSwitchListener.onSwitched(mLoadedViews.get(mCurrentBufferIndex), mCurrentAdapterIndex%mValideCount);
		}
		logBuffer();
	}
