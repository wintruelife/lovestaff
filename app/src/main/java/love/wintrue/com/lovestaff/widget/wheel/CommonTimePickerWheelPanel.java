...
import love.wintrue.com.lovestaff.widget.wheel.adapter.MinsNumberLableWheelAdapter;
...
public class CommonTimePickerWheelPanel {
    ...
    public CommonTimePickerWheelPanel(Context context) {
        mHasSelectTime = false;
        mHasSelectmin = false;
        init(context);
    }
    public CommonTimePickerWheelPanel(Context context,boolean mHasSelectmin, boolean hasSelectTime, boolean mHasSelectDay) {
        this.mHasSelectmin = mHasSelectmin;
...
//		mWheelViewMonth.setLabel("月");
    ...
    //		mWheelViewDay.setLabel("日");
		...
		if(mHasSelectTime){
			...
			mWheelViewHours.setViewAdapter(getMinsAdater(mContext, 0, 23,"时",false));
...
//			mWheelViewHours.setLabel("时");// 添加文字
			...
			if(mHasSelectmin){
				...
				mWheelViewMins.setViewAdapter(getMinsAdater(mContext, 0, 3,"分",true));
...
//				mWheelViewMins.setLabel("分");// 添加文字
    ...
    public MinsNumberLableWheelAdapter getMinsAdater(Context context, int start, int end, String lable, boolean isMins){
        MinsNumberLableWheelAdapter adapter = new MinsNumberLableWheelAdapter(context, start, end,isMins);
        adapter.setItemResource(R.layout.widget_wheel_item);
        adapter.setItemTextResource(R.id.name);
        adapter.setLable(lable);
        return adapter;
    ...
    public int[] getTime(){
        int[] timeStr = new int[2];
        timeStr[0] = mWheelViewHours.getCurrentItem();

        int mins = 0;
        int temp = mWheelViewMins.getCurrentItem()%4;
        if(temp == 0){
            mins = 0;
        }else if(temp == 1){
            mins = 15;
