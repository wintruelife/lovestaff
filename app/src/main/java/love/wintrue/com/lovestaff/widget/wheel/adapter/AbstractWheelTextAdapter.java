        ...
        if (index >= 0 && index < getItemsCount()) {
            ...
            if (textView != null) {
                ...
                if(index == mSelectedIndex){
                    textView.setTextColor(0xfff03535);
                    if(text.endsWith("年") || text.endsWith("月") || text.endsWith("日")) {
                        SpannableString span = new SpannableString(text);
                        LogUtil.e("length:"+text.length());
                        span.setSpan(new AbsoluteSizeSpan(12), text.length()-1,text.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                        text = span.toString();
                    }
                }else{
